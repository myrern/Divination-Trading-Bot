...
class TradingBot:
    ...
    def calculate_wait_time(self, last_candle, start_candle_fetch = 30):
        ...
        wait_time = (next_candle_time - current_time).seconds() - start_candle_fetch
        return max(wait_time, 0)    
    
    def _fjfjf_run_bot(self):
        
        while True:
            candles, status_code = self.get_candles(self.instrument, self.granularity, 1)
            ...
            wait_time = self.calculate_wait_time(last_candle_time, self.granularity)
            ...
            while True:
                ...
                if status_code != 200 or last_complete_candle.empty:
                    logging.error("Error fetching the last complete candle.")
                    time.sleep(10)  # Short wait before retrying
                    continue

                if pd.to_datetime(last_complete_candle.name) != pd.to_datetime(last_candle_time):
                    # New candle has been fetched, proceed with your logic
                    last_candle_time = last_complete_candle.name
                    break

                # If the new candle is not yet available, retry after a brief wait
                time.sleep(5)
...
bot = TradingBot("EUR_USD", "M5", True, 5)
